...
### `GmsCore support`:
Exclude if you are doing a ROOT YouTube / YouTube Music installtion.

> Using CLI: `-e "GmsCore support"`

### `Custom branding icon YouTube` / `Custom branding icon YouTube Music`:

Include if want a custom app icon. Exclude if you want the official app icon.

※ When included, you can choose the custom icon by opening `options.json` in an editor. (Available values: `MMT`, `Revancify Blue`, `Revancify Red`, or you can enter the absolute path where the icon file exists.)

> Using CLI: 
> - For custom app icon: `-i "Custom branding icon YouTube"` / `-i "Custom branding icon YouTube Music"`
> - For official app icon: `-e "Custom branding icon YouTube"` / `-e "Custom branding icon YouTube Music"`
